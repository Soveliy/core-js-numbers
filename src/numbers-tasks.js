...
function isPowerOfTwo(num) {
  let returnNum = num;
  while (returnNum % 2 === 0 || returnNum > 1) {
    returnNum /= 2;
  }
  if (returnNum === 1) {
    return true;
...
function toExponential(number, fractionDigits) {
  const numberExp = number.toExponential(fractionDigits);
  return numberExp;
}
...
function toFixed(number, fractionDigits) {
  const fixNumber = number.toFixed(fractionDigits);
  return fixNumber;
}
...
function toPrecision(number, precision) {
  const precNumber = number.toPrecision(precision);
  return precNumber;
}
